...
bst *createTreeBalanced(bst *root, void **array, int inizio, int fine, FUNC funz)
{
  ...
  {
    root = insertBst(root, array[medio], funz);
    createTreeBalanced(root, array, inizio, medio, funz);
    createTreeBalanced(root, array, medio+1, fine, funz);
  }
  return root;
}

// Elimina elem in input tra due livelli definiti del tree
bst *delTwoLv(bst *root, int p1, int p2, void *a, void *b, FUNC funz)  // funzionalità non implementata
{ 
  /*if(root != NULL)
  {
    if(p2>0)
