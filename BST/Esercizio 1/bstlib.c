...
{ int conf;
  if(root == NULL)
  {
    bst* nodo = AllocNode(nodo);
    nodo->key = string;
    nodo->dx = NULL;
    nodo->sx = NULL;
    return nodo;
  }else {
  	conf=strcmp(root->key,string);
  	if( conf < 0){
...
bst *duplication(bst *root)
{
  ...
  {
  	bst *nodo= AllocNode(nodo);
    nodo->key = (char*)calloc(sizeof(char),(strlen(root->key)+1));
    strcpy(nodo->key, root->key);
    nodo->sx=duplication(root->sx);
    nodo->dx=duplication(root->dx);
    return nodo;
  }
  return root;
}
...
bst *createTreeBalanced(bst *root, char **array, int inizio, int fine)
{
  ...
  if(inizio < fine&&medio>=0&&medio < fine)
  {
    ...
    nodo->sx =  createTreeBalanced(root, array, inizio, medio);
    nodo->dx =  createTreeBalanced(root, array, medio+1, fine);
    return nodo;
  }
  return root;
}
