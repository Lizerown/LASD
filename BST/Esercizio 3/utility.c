...
void printTree(bst **treeArr, int m, FUNC funz){
  ...
  switch (s1) {
    ...
    {
      int stampa = 0, i=0;
      printf("Tipo Stampa:\n1- inOrder_iterativo\n2- postOrder_iterativo\n3- preOrder_iterativo\n4- Tutte e 3\n=> ");
      scanf("\n\n%d", &stampa);
      ...
      switch(stampa)
      {
        ...
        case 1:
        {
          for(i = 0; i<m; i++)
          {
            printf("\nTree n: %d\n", i+1);
            if(treeArr[i]!=NULL)
              inOrder_iterativo(treeArr[i],funz);
            else
              printf("Tree vuoto\n");
          }
        }break;
        ...
        case 2:
        {
          for(i = 0; i<m; i++)
          {
            printf("\nTree n: %d\n", i+1);
            if(treeArr[i]!=NULL)
              postOrder_iterativo(treeArr[i], funz);
            else
              printf("Tree vuoto\n");
          }
        }break;
        ...
        case 3:
        {
          for(i = 0; i<m; i++)
          {
            printf("\nTree n: %d\n", i+1);
            if(treeArr[i]!=NULL)
              preOrder_iterativo(treeArr[i],funz);
            else
              printf("Tree vuoto\n");
          }
        }break;
      ...
      switch(stampa)
      {
        ...
        case 1:
        {
          if(treeArr[s2-1]!=NULL)
            inOrder_iterativo(treeArr[s2-1], funz);
          else
            printf("Tree vuoto\n");
        }break;
        // POST ORDER
        ...
        {
          if(treeArr[s2-1]!=NULL)
            postOrder_iterativo(treeArr[s2-1],funz);
          else
            printf("Tree vuoto\n");
        }break;
        // PRE ORDER
        ...
        {
          if(treeArr[s2-1]!=NULL)
            preOrder_iterativo(treeArr[s2-1], funz);
          else
            printf("Tree vuoto\n");
        }break;
        // Stampa tutto
...
void delTree(bst **treeArr, int m, FUNC funz)
{
  ...
  switch (s1) {
    ...
    case 2:
    {
     ...
     treeArr[s2-1] = deallocTree_iterativo(treeArr[s2-1],funz);
      treeArr[s2-1] = NULL;
    }break;
  }
}
...
void deleteNode(bst **treeArr, int m, FUNC funz){
  ...
  if(treeArr[s1-1] != NULL){
    inOrder_iterativo(treeArr[s1-1],funz);
    printf("\n Elemento da rimuovere: \n");
    void *elem = funz.getInput();
    treeArr[s1-1] = delNode_iterativo(treeArr[s1-1], elem, funz);
    if(treeArr[s1-1] != NULL){
...
bst *dupTree(bst **treeArr, int m, int *lenTree, FUNC funz){
  ...
  root = duplication_iterativo(treeArr[s1-1], funz);
  checkEquality_iterativo(treeArr[s1-1], root, &j, funz);
  if(j == 1)
    printf("duplicazione efffettuata\n");
  else
    printf("duplicazione non effettuata\n");
...
void equality(bst **treeArr, int m, FUNC funz){
  ...
  checkEquality_iterativo(treeArr[s1-1], treeArr[s2-1], &j, funz);
  if(j == 1)
    printf("Tree uguali\n");
  else
...
void treeArray(bst **treeArr, int m, int *lenTree, FUNC funz){
  ...
  if(lenTree[s1-1] != 0)
  {
    ...
    treeToArray_iterativo(treeAr, treeArr[s1-1], &pos, funz);
    printf("Stampare?(y/n): ");
    scanf("\n\n%c", &check);
    clsBuffer();
...
bst *treeBalanced(bst **treeArr, int m, int *lenTree, FUNC funz){
  ...
  if(lenTree[s1-1] != 0)
  {
    ...
    treeToArray_iterativo(treeAr, treeArr[s1-1], &pos, funz);
    roott = createTreeBalanced_iterativo(treeAr, pos, funz);
...
void deleteTwoLv(bst **treeArr, int m,FUNC funz){
  ...
  if(alt >= 0){
    ...
    treeArr[s1-1] = delTwoLv_iterativo(treeArr[s1-1], p1, p2, a, b, funz);
	printf("\n Tree aggiornato \n");
    inOrder_iterativo(treeArr[s1-1],funz);
  }else
    printf("Tree vuoto\n");
